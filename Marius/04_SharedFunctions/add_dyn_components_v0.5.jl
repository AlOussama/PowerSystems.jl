...
for x in 1:anzahl_re_gen
    ...
    re_bus = get_bus(re_stat_gen)
    println("================")
    re_bustype = get_bustype(re_bus);
    re_bustype_str = string(re_bustype)
    split_str_re_bustype = split(re_bustype_str," ")
    _re_bustype = split_str_re_bustype[1]
    # println(_re_bustype)
    if _re_bustype == "PQ"
