...
inverter = DynamicInverter(
    ...
    ω_ref = 1.0, # ω_ref,
    converter = re_converter,#AverageConverter(rated_voltage = 380, rated_current = 100.0),
    outer_control = re_outer_control,#OuterControl(
    #     VirtualInertia(Ta = 2.0, kd = 400.0, kω = 20.0),
    #     ReactivePowerDroop(kq = 0.2, ωf = 1000.0),
    # ),
    inner_control =re_inner_control, #CurrentControl(
    #     kpv = 0.59,     #Voltage controller proportional gain
    #     kiv = 736.0,    #Voltage controller integral gain
    #     kffv = 0.0,     #Binary variable enabling the voltage feed-forward in output of current controllers
    #     rv = 0.0,       #Virtual resistance in pu
    ...
    #     kad = 0.2,
    # ),
    dc_source = re_dc_source, #FixedDCSource(voltage = 380.0),
    freq_estimator =re_freq_estimator,# KauraPLL(
    #     ω_lp = 500.0, #Cut-off frequency for LowPass filter of PLL filter.
    #     kp_pll = 0.084,  #PLL proportional gain
    #     ki_pll = 4.69,   #PLL integral gain
    # ),
